...
class My_ParseRestClient {
	...
	public function __construct(){
		$this->_appid = My_Core::getConfig()->parse->appid;
    	$this->_masterkey = My_Core::getConfig()->parse->masterkey;
    	$this->_restkey = My_Core::getConfig()->parse->restkey;
    	$this->_parseurl = My_Core::getConfig()->parse->parseurl;
	...
	private function checkResponse($response,$responseCode,$expectedCode){
		...
		//TODO: Ii the reponse is an array(batch requests) - every element sholud be checked for success
		if(!in_array($responseCode,$expectedCode))
		{
			$response = json_decode($response);
			$response->request_successfull = false;
			return $response;
		}
		else
		{
			//check for empty return
			if ($response == '{}' || is_array(json_decode($response)) || is_null(json_decode($response))){
				return true;
			}
			else
			{
				$response = json_decode($response);
				$response->request_successfull = true;
				return $response;
			}
		}
	}
