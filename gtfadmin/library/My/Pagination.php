...
if(!class_exists("My_Pagination")):	// Prevent multiple class definitions
...
class My_Pagination
{
	...
	*******************************************************/
	...
	public $_totalEntries         = null;		// Total number of entries (usually supplied by MySQL query)
	private $_pagesPerSection      = null;		// Total number of pages displayed per section (supplied by admin)
	public $_entriesPerPage       = null;		// Total number of entries displayed per page (supplied by user)
	public $_currentPage          = null;		// Current page viewed by user
	public $_fallbackLimit		 = 1000;
	...
	*******************************************************/
	...
	function __construct($totalEntries, $pagesPerSection, $options = "", $paginationID = "paginator", $stylePageOff = "pageOff",
						 $stylecurrent = "current", $styleErrors = "paginationErrors", $styleSelect = "paginationSelect")
	{
		...
		$this->setStyles($stylePageOff, $stylecurrent,
...
//		$output .= '#'. $this->getPaginationID();	// Add #xxx at the end of URL for auto-scrolling
	...
	private function _navBox($text, $destinationPage, $end = 0)		// returns span-encased link for pagination bar
	{
		...
		// Determine Style
		$style = ($this->_currentPage == $destinationPage && !$end) ? $this->_styles["current"] : $this->_styles["pageOff"];
		...
		{
			$text = '<a class="button small" href="'. $this->_getURL($destinationPage) .'">'. $text .'</a>'; // then make $text a link

            $onClick = ($url) ? "onclick=\"location.href='". $url ."'\"" : "";    // Determine if span element is clickable

            $final_element = '<li ' . $title .' '. $onClick .'>'. $text .'</li>';
		} else {

            if($this->_currentPage == $destinationPage && $this->_currentPage > 1)
            	$final_element = '<li><span>'. $text .'</span><li>';	
            else
            	$final_element = '';
        }
...
//			return $this->displayErrors();
		...
		//$output  = $this->_navBox("<i class='icon-backward'></i> first", $firstPage, 1);	// First page
		$output = $this->_navBox("Prev", $previousPage, 2);		// Previous page
		if ($output != '')
		{
		...
		$output .= $this->_navBox("Next", $nextPage, 3);			// Next Page
		//$output .= $this->_navBox("<i class='icon-forward'></i> last", $lastPage, 4);		// Last Page
	...
	public function displaySelectInterface()		// displays the <select> interface for choosing display amount
	{
		...
		$output = '
		<form name="'. $formName . $formNumber .'" method="post" style="display:inline-block;" class="custom" action="'. $this->_getURL($this->_currentPage) .'">
		<select style="margin-bottom:0px" name="'. $this->_getPOSTVarName() .'" OnChange ="'. $formName . $formNumber .'.submit()">';

		foreach($this->_displayOptions as $line)
		{
			if($this->_entriesPerPage == $line || $this->_entriesPerPage == $this->_totalEntries || $this->_entriesPerPage == $this->_fallbackLimit)	// If option equals entries per page
	...
	*******************************************************/
	...
	public function setStyles($pageOff = "", $current = "", $select = "", $errors = "")	// sets CSS style class names
	{
		$this->_styles = array(
			"pageOff"  => $pageOff,
			"current"   => $current,
